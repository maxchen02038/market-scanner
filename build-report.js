...
async function main() {
  ...
  for (let i = 0; i < allStocks.length; i += 10) {
    ...
    quoted.push(...results.filter(s => s && s.c && (s.dp??0) > 0)); // pre-filter momentum>0
    await sleep(600);
  }
  console.log(quoted.length, 'stocks passed the positive-momentum pre-filter');

  // 4. Score the filtered set (batches of 3, 1.2s apart for the rate limit)
  const scored = [];
  for (let i = 0; i < quoted.length; i += 3) {
    ...
    if (i % 30 === 0) console.log('Scored', scored.length, '/', quoted.length);
    await sleep(1200);
  }
